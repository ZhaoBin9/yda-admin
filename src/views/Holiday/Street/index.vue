...
import { deleteArticleData } from '@/apis/article'
import { getClassDataInfo } from '@/apis/class'
import { getList as getClassList } from '@/apis/class'
...
export default {
  ...
  methods: {
      ...
      getClassDataInfo({
        ...
        if (code === 0) {
          this.listData = data?.map(item => {
            const child = this.classes.find(o => item.classId === o.id) || {}
            item.parentName = child?.name || ''
            return item
          })
    ...
    infoInit(data) {
      const { id, imageId, fullImage, content, classId, title, top, online, summary, sortIndex } = data || {}
      const form = {
